...
public class EjercicioBaloncestoExcel extends javax.swing.JFrame {
    ...
    private void botonInsertarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_botonInsertarActionPerformed
        ...
        calcularMediasPorEquipo(nombreArchivo);
    ...
    private void calcularMediasPorEquipo(String rutaArchivo) {

        File archivo = new File(rutaArchivo);
        Workbook libroTrabajo = null;

        try (FileInputStream fileInputStream = new FileInputStream(archivo)) {
            libroTrabajo = new XSSFWorkbook(fileInputStream);
        } catch (IOException e) {
        ...
        int borrarHojaMedias = libroTrabajo.getSheetIndex("Medias");
        if (borrarHojaMedias != -1) {
            libroTrabajo.removeSheetAt(borrarHojaMedias);
        }

        Sheet hojaMedias = libroTrabajo.createSheet("Medias");
        ...
        for (int i = 0; i < nombresEncabezados.length; i++) {
            encabezado.createCell(i).setCellValue(nombresEncabezados[i]);
        }

        int filaMedia = 1;


        for (int i = 0; i < libroTrabajo.getNumberOfSheets(); i++) {
            Sheet hojaJugador = libroTrabajo.getSheetAt(i);
            String nombreJugador = libroTrabajo.getSheetName(i);

            if ("Medias".equals(nombreJugador)) {
                continue;
            }

            Row filaMedias = hojaMedias.createRow(filaMedia++);

            // Coloca el nombre del jugador en la primera celda de la fila
            filaMedias.createCell(0).setCellValue(nombreJugador);

            int ultimaFila = hojaJugador.getLastRowNum();

            double[] sumas = new double[16];
            int[] cantidades = new int[16];

            // Empezar desde la fila 1 para evitar el encabezado
            for (int fila = 1; fila <= ultimaFila; fila++) {
                Row filaDatos = hojaJugador.getRow(fila);
                if (filaDatos != null) {
                    for (int columna = 0; columna < 16; columna++) {
                        Cell celda = filaDatos.getCell(columna);
                        if (celda != null && celda.getCellType() == CellType.NUMERIC) {
                            sumas[columna] += celda.getNumericCellValue();
                            cantidades[columna]++;
                        }
                    }
                }
            ...
            for (int j = 0; j < 16; j++) {
                double media = (cantidades[j] > 0) ? sumas[j] / cantidades[j] : 0;
                filaMedias.createCell(j + 1).setCellValue(media);
            }
        }
